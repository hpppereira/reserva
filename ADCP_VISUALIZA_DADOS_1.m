...
%% Observação dos dados do ADCP Aquadopp
%Douglas NEMES
%Praia da Reserva
%ADCP AQUADOP NORTEK
...
Med_v1c1= mean(v1c1);          % Média de V1
v1c1M=v1c1-mean(v1c1);         % tirando a Média de V1
Medv1c1=v1c1-Med_v1c1;         % a média ponto a ponto da serie

%V1 camada 2
v1c2=Dados_v1(:,2);           % V1 camada 2 East
Med_v1c2= mean(v1c2);          % Média de V1
v1c2M=v1c2-mean(v1c2);         % tirando a Média de V1
Medv1c2=v1c2-Med_v1c2;

%V1 camada 3
v1c3=Dados_v1(:,3);           % V1 camada 3 East
Med_v1c3= mean(v1c3);        % Média de V1
v1c3M=v1c3-mean(v1c3);         % tirando a Média de V1
...
Med_v2c1= mean(v2c1);          % Média de V1
v2c1M=v2c1-mean(v2c1);         % tirando a Média de V2
Medv2c1=v2c1-Med_v2c1;         % a média ponto a ponto da serie

%V2 camada 2
v2c2=Dados_v2(:,2);           % V1 camada 2 North
Med_v2c2= mean(v2c2);          % Média de V1
v2c2M=v2c2-mean(v2c2);         % tirando a Média de V2
Medv2c2=v2c2-Med_v2c2;         % a média ponto a ponto da serie

%V2 camada 3
v2c3=Dados_v2(:,3);           % V2 camada 3 North
Med_v2c3= mean(v2c3);        % Média de V2
v2c3M=v2c3-mean(v2c3);         % tirando a Média de V2
Medv2c3=v2c3-Med_v2c3;         % a média ponto a ponto da serie

load ('ADCP_05_08_2015v3.v3')
%V3 camada 1
Dados_v3 = ADCP_05_08_2015v3;
v3c1=Dados_v3(:,1);           %  V3 Up Z
Med_v3c1= mean(v3c1);          % Média de V3
v3c1M=v3c1-mean(v3c1);         % tirando a Média de V3

%V3 camada 2
v3c2=Dados_v3(:,2);           % V3 camada 2 UP
Med_v3c2= mean(v3c2);          % Média de V1
v3c2M=v3c2-mean(v3c2);         % tirando a Média de V2
%V3 camada 3
v3c3=Dados_v3(:,3);           % V3 camada 3 UP
Med_v3c3= mean(v3c3);        % Média de V2
v3c3M=v3c3-mean(v3c3);         % tirando a Média de V2
...
title ('Maré e Ondas (Pressão.wad)')
subplot(3,1,2)
plot(Press_Sen-mean(Press_Sen))
title('Maré (Pressão.sen)') 
H=PressWad-mean(PressWad) %tirando a média
%H= pressao-mean (pressao); %n elevaçlão da Sup do Mar 
mare=smooth(H,2000); %tiro a maré
Onda=H-mare;    %tiro a maré do nivel
subplot(3,1,3)
plot(Onda)
title ('Ondas')
...
%% CALCULO DA CORRENTE
zcorr=0; %denomimando a primeira celula (fora da água - ajustavel)
...
z= squeeze(V(:,2,:));   % posição da célula
p= E(:,14)';            % prof agua (sensor de pressão)
...
c_val=boolean(z<pz); %celulas válidas - celulas dentro da água 
                     % de acordo com o sensor de pressão
z_val=z*NaN;         % celulas que estão fora terão valor de NaN
z_val(c_val)=z(c_val); % posição das celulas validas

ve_val=ve*NaN;          %  posição das celulas leste validas
ve_val(c_val)=ve(c_val);% velocidade leste -  celulas válidas

vn_val=vn*NaN;           %  posição das celulas norte validas
vn_val(c_val)=vn(c_val); % velocidade norte -  celulas válidas

vm_val=vm*NaN;           %  posição das celulas modulo da vel validas
vm_val(c_val)=vm(c_val); % velocidade modulo da vel -  celulas válidas

cs=sum(c_val);           % celulas superficial de acordo com a pressão
zcs=max(z_val);

vecs=cs*NaN; % valores da velocidade superficial leste de acordo com o sensor de pressão
vncs=cs*NaN; % valores da velocidade superficial norte de acordo com o sensor de pressão
vmcs=cs*NaN; % valores do modulo da velocidade superficial de acordo com o sensor de pressão
...
% inverter esse gráfico no editor
figure(8)
