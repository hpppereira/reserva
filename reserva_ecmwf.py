...
#pathname = os.environ['HOME'] + '/Dropbox/reserva/modelagem/ECMWF/'
pathname = 'C:/Users/douglas.MENTAWAII/Dropbox/nemes/modelagem/ECMWF/'
#carrega dados do ECMWF
#dd1 = nc.Dataset(pathname + 'Ondas_ECMWF_SEAL_Eventos.nc')
dd = nc.Dataset(pathname + '_grib2netcdf-atls00-95e2cf679cd58ee9b4db4dd119a05a8d-v_FVis.nc')

#dd3 = nc.Dataset(pathname + 'Ondas_ECMWF_SEAL_2008_AXYS.nc')

#aa = xray.open_dataset(pathname + '_grib2netcdf-atls00-95e2cf679cd58ee9b4db4dd119a05a8d-v_FVis.nc')
...
adcp = pd.read_csv('out/teste.csv', parse_dates=['date'],index_col=['date'])
...
df = pd.DataFrame(np.array([date,hs[:,0,0],tp[:,0,0],dp[:,0,0]]).T, columns=['date','hs','tp','dp'])
